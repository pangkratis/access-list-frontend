<template>
  <div>
     <div class="q-pa-md" style="width: 1000px; height: 500px">
      <q-table
        title="Days Off History"
        :data="pagDaysOff.data"
        :columns="columns"
        :pagination.sync="pagination"
        @request="onRequest"
        row-key="name"
        class="fixed-center rounded-borders shadow-9"
...
import { getPagDaysOff } from '../assets/gql/queries/dayoff'
  ...
  setup (_, { root }) {
    ...
    const variables = computed(() => {
      return {
        ...
        ilimit: pagination.value.rowsPerPage
      }
    })

    const { result: getPagDaysOffResult, onResult } = useQuery(getPagDaysOff, variables)

    onResult(() => {
      pagination.value.rowsNumber = getPagDaysOffResult.value.getPagDaysOff.total
    })
    const pagDaysOff = computed(() => {
      return getPagDaysOffResult.value ? getPagDaysOffResult.value.getPagDaysOff : []
    })
